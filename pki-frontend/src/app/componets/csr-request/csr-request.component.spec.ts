...
export class CsrRequestComponent implements OnInit {
  ...
  @ViewChild('privateKeyFileInput') privateKeyFileInput!: ElementRef;
 ...
 */
    ...
    this.csrForm.get('selectedCaId')?.valueChanges.subscribe(caId => {
      if (caId) {
        ...
        if (selectedCA) {
          ...
          this.maxDuration = diff > 0 ? diff : 1;
  ...
  onCSRFileChange(event: any): void {
  ...
  onPrivateKeyFileChange(event: any): void {
    const file = event.target.files[0];
    if (file) {
      // Proveri veličinu fajla (max 1MB)
      if (file.size > 1024 * 1024) {
...
/*
    this.csrService.uploadCSR(formData).subscribe({
      next: (res) => {
        ...
        this.successMessage = 'CSR uploaded successfully!';
